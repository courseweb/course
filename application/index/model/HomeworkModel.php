...
{
    public function showBlankAns($homework_th,$class_id){
        $question = array();

        $join = [
            ['question b','b.homework_id=a.homework_id']
        ];
        $result = Db::table('homework')->alias('a')->join($join)->where('a.homework_th',$homework_th)->where('a.class_id',$class_id)->where('b.type',4)->field('b.ques_th,b.question,b.answer,b.grade')->order('b.ques_th')->select();
        if(!$result){
            $result = 'failure';
        }else{
            ...
            $result = Db::table('homework')->alias('a')->join($join)->where('a.homework_th',$homework_th)->where('a.class_id',$class_id)->where('b.type',4)->field('d.stu_id,d.stu_name,c.ques_th,c.answer')->order('b.ques_th')->distinct(true)->select();
            if(!$result){
                $result = 'failure';
            }else{
                foreach ($result as $index => $set) {
