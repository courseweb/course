angular.module('login_info', ['ui.router']).controller('login_info_controller', ['$scope', '$log', '$rootScope', '$state', function($scope, $log, $rootScope, $state) {
    $scope.$log = $log;
    ...
    //保存动态加载到导航栏的内容（头像和铃铛）
    $scope.dynamic_bar = "view/profile_unlogin.html";
        ...
        $rootScope.sendData("/index.php/index/Index/loginUser", fd, function() {
            ...
            if ($scope.return === "success") //三个等号表示值和变量类型都相等
            {
                ...
                if ($scope.user.type === "2") //教师
                {
                    ...
                    $scope.dynamic_bar = "view/profile_login_teacher.html";
                ...
                } else if ($scope.user.type === "3") //管理员
                {
                    ...
                    $scope.dynamic_bar = "view/profile_login_admin.html";
    ...
    $rootScope.editStuInfo = function() {
        ...
        $rootScope.sendData("/index.php/index/Index/loginUser", fd5, function() {
            if ($rootScope.xmlhttp.readyState == 4 && $rootScope.xmlhttp.status == 200) {
                $scope.return = JSON.parse($rootScope.xmlhttp.responseText).result;
            }
    ...
    $rootScope.editStuCode = function() {
        ...
        fd6.append("newPwd", $scope.edit_stu_info.newPwd);
        $rootScope.sendData("/index.php/index/Index/loginUser", fd6, function() {
            if ($rootScope.xmlhttp.readyState == 4 && $rootScope.xmlhttp.status == 200) {
                $scope.return = JSON.parse($rootScope.xmlhttp.responseText).result;
            }
    ...
    $rootScope.setNewPwd = function(){
       ...
       else
       {
           ...
           if($scope.return === "success"){
             ...
             $scope.$apply(function() {
                alert("重置密码失败！");
                $scope.findbackPwd_user.result_of_modify = "重置密码失败！";
