app.controller('t_navigation_controller',function($scope,$rootScope, $state){
...
app.controller('t_check_info_controller', ['$scope', '$log', '$rootScope', function($scope,$log,$rootScope){
  $scope.notice_time=0;
    $scope.$log=$log;
...
app.controller('t_publish_info_controller', ['$scope', '$log', '$rootScope', function($scope,$log,$rootScope){
  if (window.XMLHttpRequest) {//XHR创建对象
        // IE7+, Firefox, Chrome, Opera, Safari 代码
        $rootScope.xmlhttp = new XMLHttpRequest();
  }
...
app.controller('t_check_xinde_controller', ['$scope', '$log', '$rootScope', function($scope,$log,$rootScope){
  $scope.xinde_time=1;
...
app.controller('t_publish_xinde_controller', ['$scope', '$log', '$rootScope', function($scope,$log,$rootScope){
  if (window.XMLHttpRequest) {//XHR创建对象
        // IE7+, Firefox, Chrome, Opera, Safari 代码
        $rootScope.xmlhttp = new XMLHttpRequest();
  }
...
app.controller('t_upload_material_controller', ['$scope', '$log', '$rootScope', function($scope,$log,$rootScope){
...
app.controller('t_publish_homework_controller',function($scope,$rootScope){
  $scope.a={date:""};
  ...
  $scope.nextChooseQuestion=function(chooseScore,chooseQuestion,chooseAnswerA,chooseAnswerB,chooseAnswerC,chooseAnswerD){
    if($scope.questionID<$scope.homeworkQuestion.length){//如果这题已经被编辑过了，显示已填写的值
      ...
      if($scope.homeworkQuestion[$scope.questionID].questionType==1){//说明是选择题
        ...
        var t = document.getElementById("chooseQuestionType");
      ...
      $scope.homeworkQuestion[tempQuestionID]={questionType:1,chooseScore:chooseScore,chooseQuestion:chooseQuestion,chooseAnswerA:chooseAnswerA,chooseAnswerB:chooseAnswerB,chooseAnswerC:chooseAnswerC,chooseAnswerD:chooseAnswerD,rightQuestionAnswer:rightQuestionAnswer};
      $scope.chooseScore="";
  ...
  $scope.beforeChooseQuestion=function(chooseScore,chooseQuestion,chooseAnswerA,chooseAnswerB,chooseAnswerC,chooseAnswerD){
    var temp=$("#chooseHomeworkCorrectAnswer option:selected");
    var rightQuestionAnswer=temp.val();
    $scope.homeworkQuestion[$scope.questionID]={questionType:1,chooseScore:chooseScore,chooseQuestion:chooseQuestion,chooseAnswerA:chooseAnswerA,chooseAnswerB:chooseAnswerB,chooseAnswerC:chooseAnswerC,chooseAnswerD:chooseAnswerD,rightQuestionAnswer:rightQuestionAnswer};
  ...
  $scope.nextFillInQuestion=function(fillInScore,fillInQuestion,fillInAnswer){
    if($scope.questionID<$scope.homeworkQuestion.length){//如果这题已经被编辑过了，显示已填写的值
      ...
      var tempQuestionID=$scope.questionID;
      $scope.homeworkQuestion[tempQuestionID]={questionType:2,fillInScore:fillInScore,fillInQuestion:fillInQuestion,fillInAnswer:fillInAnswer};
      $scope.fillInScore="";
  ...
  $scope.beforeFillInQuestion=function(fillInScore,fillInQuestion,fillInAnswer){
    ...
    $scope.homeworkQuestion[$scope.questionID]={questionType:2,fillInScore:fillInScore,fillInQuestion:fillInQuestion,fillInAnswer:fillInAnswer};
    if($scope.questionID>0)
      $scope.questionID=$scope.questionID-1;
    if($scope.homeworkQuestion[$scope.questionID].questionType==1){//说明是选择题
      $scope.fillInQuestionPanel=true;
...
app.controller('t_correct_homework_controller',function($scope,$rootScope){
  $scope.studentOnlineAnswer = [
    {studentID:1,studentName:'陈毅强',questionID:'1',studentAnswer:'啦啦啦'},
    {studentID:2,studentName:'谢思宇',questionID:'1',studentAnswer:'萌萌'}
  ];
  $scope.studentReport = [
    {studentID:1,studentName:'陈毅强',URL:'http://'},
    {studentID:1,studentName:'谢思宇',URL:'http://'}
  ];
});
